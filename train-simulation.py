...
def train(params):
    ...
    train_dataset = SimulationDataset(params.data_path, 'train', params.category)
    val_dataset = SimulationDataset(params.data_path, 'valid', params.category)
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        # training
        model.train()
        for i, (p, c, label) in enumerate(train_dataloader):
            p, c, label = p.to(params.device), c.to(params.device), label.to(params.device)
        ...
        with torch.no_grad():
            ...
            for i, (p, c, label) in enumerate(val_dataloader):
                p, c, label = p.to(params.device), c.to(params.device), label.to(params.device)
                coarse_pred, dense_pred = model(p)
                ...
                if rand_iter == i:
                    ...
                    input_pc = p[index].detach().cpu().numpy()
                    coarse_pc = coarse_pred[index].detach().cpu().numpy()
                    dense_pc = dense_pred[index].detach().cpu().numpy()
                    gt_pc = c[index].detach().cpu().numpy()


                    plot_pcd_one_view(os.path.join(epochs_dir, 'epoch_{:03d}.png'.format(epoch)),
                                      [input_pc, coarse_pc, dense_pc, gt_pc],
                                      ['Input', 'Coarse', 'Dense', 'Ground Truth'], xlim=(-5.3, 0.35), ylim=(-2.3, 0.35), zlim=(-1, 0.35))
...
# python train-simulation.py --exp_name simulation-test --lr 0.00005 --epochs 400 --batch_size 16 --coarse_loss cd --num_workers 8 --data_path /data/puwong/pcn/data/data/SimulationDataset/robot-single